...
prevalent framework currently used to implement these functionalities is the Vision-Language-Action (VLA) 
architecture. Building on the recent advances in multimodal reasoning achieved by Vision-Language Models (VLMs),
researchers have extended multimodal learning to robotic control, giving rise to VLA systems.

VLA models (e.g., OpenVLA, Octo, Nora, Otter) take as input visual observations (such as images or video frames) 
...
In a recent architecture proposed by Google, Gemini Robotics 1.5, a VLA model works in conjunction with
...
for executing these simpler commands. Despite their impressive performance, current VLA architectures—including
...
\begin{enumerate}
    ...
    performance in such scenarios, consistent with the “less-is-more” hypothesis [cite]. In the 
    ...
    requires planning at multiple levels of abstraction. This, in turn, presupposes the existence of 
    ...
    In current VLA architectures, however, the hierarchical organization of sensory, motor, and 
    planning representations is not explicitly defined but it should rather emerge implicitly from 
    end-to-end gradient-based training on large datasets. Consequently, these models may lack the 
    structural inductive biases necessary for scalable, hierarchical planning and control.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Reduced reliance on labeled data.} By leveraging unsupervised learning, the model will require far fewer teleoperated trajectories, thereby lowering the cost and effort of dataset collection.
    \item \textbf{Accelerated exploration of the action space.} The entanglement between sensory and motor representations will allow the system to rapidly identify promising motor strategies directly from sensory cues.
    \item \textbf{More efficient and scalable planning.} Hierarchical abstraction will enable the model to plan over longer temporal horizons while maintaining low prediction error, supporting efficient adaptation to novel tasks and environments.
\end{itemize}
...
\paragraph{Experimental Setup: Simulation Environment}
We will use the MuJoCo physics engine to simulate the robot’s behavior in a realistic workspace. The environment will include:
...
In this part of the project, the main contribution is not modifying the V-JEPA architecture, but determining which robot experiences (i.e., which actions) should be collected to develop an action-oriented representation of the world.
To investigate this, we will fine-tune the V-JEPA model using actions collected from a single robot.
The robot will perform predefined actions. Such actions will involve: moving the end-effector without any object involved, simple interaction with objects (accidentally touching, pushing, pulling), complex object manipulation (piling objects, precision picking). 


All robot sensors (proprioceptive and exteroceptive) will be recorded while the robot performs the actions.


The V-Jepa architecture will be fine-tuned using the same training paradigm used in (cite LeCun paper):
\begin{itemize}
    \item Sensor input will be passed to the encoder to create a representation of the sensory input.
    \item A predictor will be trained to predict the next representation of the sensors using as input the action performed and the encoding of the current sensors. As shown in the following figure.
\end{itemize}

% Add first figure here
...
We would also like to test an architecture where the encoder becomes a recurrent architecture see figure (B).
...
Here we propose a way to use the architecture to build a high level motor planning. Our approach assumes that, within the high-level representation (the output of Encoder 2), we can separate the component that encodes the relative positions of the robot’s body parts (shown in red in the figure) from the component that encodes the external world (shown in green). In the following we will refer to the component that encodes the relative positions of the robot’s body parts as the “pose-encoding nodes”.
...
In order to validate that the previous paradigm can be effectively used for planning we will test in the following way. We will collect a new dataset in a way similar to how we collected the dataset that we used so far: robot moving hands performing simple and more complex action sequences. While the robot performs such actions we will record the value of all the sensors and the encodings (all the output of the two encoders) for different time stamps.
...
the first decoder (Decoder 2) transforms the vector $h_2$ into the lower-level vectors $l_3$ and $l_4$. A second decoder
then transforms $l_3$ and $l_4$ into the expected sensory inputs. We assume that, once we have the raw proprioceptive sensor values, 
defining the corresponding motor commands is straightforward. In this way, we obtain the full sequence of motor commands needed to reach
